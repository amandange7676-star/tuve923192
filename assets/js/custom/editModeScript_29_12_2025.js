...
function editClientSite(filesDetailsMap) {
    ...
    // Generate a cache-busting string
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            newTab.onload = function () {
                ...
                const appendCacheBustedElement = (tag, attributes, toBody) => {
                    if (attributes.href) {
                        attributes.href = appendCacheBustToUrl(attributes.href);
                    }
                    if (attributes.src) {
                        attributes.src = appendCacheBustToUrl(attributes.src);
                    }
                    appendElement(tag, attributes, toBody);
                };

                // Function to append the cache bust query string to URLs
                function appendCacheBustToUrl(url) {
                    return url.includes('?') ? `${url}&_=${CACHE_BUST}` : `${url}?_=${CACHE_BUST}`;
                }
                ...
                $('<input>', {
                    ...
                    value: filesDetailsMap["pageName"]
                }).appendTo(newTab.document.body);

                // Append the cache-busted resources
                appendCacheBustedElement('link', { rel: 'stylesheet', href: 'https://cdn.quilljs.com/1.3.6/quill.snow.css' }, false);
                appendCacheBustedElement('link', { rel: 'stylesheet', href: 'assets/css/custom/editmode.css' }, false);
                appendCacheBustedElement('link', { rel: 'stylesheet', href: 'assets/css/custom/custom.css' }, false);
                appendCacheBustedElement('script', { src: 'https://cdn.quilljs.com/1.3.6/quill.min.js', type: 'text/javascript' }, true);
                appendCacheBustedElement('script', { src: 'assets/js/custom/main.js', type: 'text/javascript' }, true);
                appendCacheBustedElement('script', { src: 'assets/js/custom/editmode.js', type: 'text/javascript' }, true);
                appendCacheBustedElement('script', { src: 'assets/js/custom/editModeScript.js', type: 'text/javascript' }, true);

                // Add 'edit-site' class to all anchor tags
                var anchorTags = newTab.document.querySelectorAll('a');
                anchorTags.forEach(function(anchor) {
                    anchor.classList.add('edit-site');
                });
